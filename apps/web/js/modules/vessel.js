...
window.ChengProModules.vessel = {
  ...
  async render(root) {
    ...
    let shared = null;
    if (active) {
      try { shared = await ChengPro.api.fetch('/api/vessels/' + encodeURIComponent(active.id)); }
      catch (e) { toast(e.message); }
    }

    const v = shared?.vessel || {
      name: '', imo: '', callSign: '', flag: '', company: '', type: '', dwt: '', notes: '',
    };
    ...
    root.innerHTML = `
      <section class="panel">
        <div class="section-head">
          <div>
            <h2>Vessel Setup</h2>
            <p>Shared ship identity for Voyage and Tanks. One form, one fetch from the server.</p>
          </div>
        </div>
        <form id="vesselForm" class="grid-2">
          ...
          <div class="field"><label>Company</label><input name="company" value="${esc(v.company || '')}"></div>
      ...
      <section class="panel">
        <div class="section-head">
          <div>
            ...
            <p>Select a vessel to make it active for both modules.</p>
          </div>
        </div>
    ...
    renderFleet(root.querySelector('#fleetTable tbody'));
    ...
    root.querySelector('#saveVessel').addEventListener('click', async () => {
      ...
      try {
        if (active && !root._forceNew) {
          await ChengPro.api.fetch('/api/vessels/' + encodeURIComponent(active.id), {
            method: 'PUT',
            body: JSON.stringify(data),
          });
          toast('Vessel saved');
        } else {
          const created = await ChengPro.api.fetch('/api/vessels', {
            method: 'POST',
            body: JSON.stringify(data),
          });
    ...
    root.querySelector('#deleteVessel')?.addEventListener('click', async () => {
      if (!active || !confirm('Delete vessel and all voyage/tank data?')) return;
      try {
        await ChengPro.api.fetch('/api/vessels/' + encodeURIComponent(active.id), { method: 'DELETE' });
        await ChengPro.vessel.refresh();
