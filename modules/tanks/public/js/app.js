...
function renderNav() {
  ...
  /* Inside ChEng AIO the theme button is on the AIO's own strip, so a second
     ...
     all. Measured: no reachable toggle at 1180x820 or 1920x1080 by any route.
     The CSS lays it out along the bar rather than down a column. */
  if (!isAioEmbedded()) {
    const themeBtn = document.createElement('button');
    themeBtn.type = 'button';
    themeBtn.className = 'theme-toggle no-print';
...
function renderResultSteps(panel, tank, r, inputs) {
  ...
  if (r.gaugeType === 'volume') {
    ...
    defs.push({
      label: 'Heel volume',
      formula: `direct volume correction · heel grid inc=${r.heelIncrement ?? '?'}`,
      value: fmt(r.heelVolume != null ? r.heelVolume : 0,3)+' m³',
    });
    defs.push({
      label: 'Trim volume',
      formula: `direct table trim → volume grid inc=${r.soundingIncrement ?? '?'}`,
      value: fmt(r.trimVolume != null ? r.trimVolume : r.volumeObserved,3)+' m³',
    });
    defs.push({
