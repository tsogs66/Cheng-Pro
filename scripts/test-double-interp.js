 ...
 *
 * Upright, the heeling table must leave that alone. It prints no zero column,
 * and reading straight from -1 to +1 across the gap used to fake a 1.174
 * correction, which the volume-correction path subtracted whole: 375.556
 * against the book's 374.381.
 */
'use strict';
...
const tankOf = (calcType) => ({
  ...
  trimAxis: ULLAGE, trimVals: TRIM_VALS, trimGrid: TRIM_GRID,
  listAxis: ULLAGE, listVals: LIST_VALS, listGrid: LIST_GRID,
});
const read = (calcType, list) => calc.computeTank(tankOf(calcType),
...
near(calc.bilinearInterpInc(ULLAGE, TRIM_VALS, TRIM_GRID, 1080, 0.41, 50),
     374.3814, 0.0005, 'trim table, ullage 1080 at 0.41 m by stem');

/* Volume-correction tanks: upright, the heel table contributes nothing. */
const direct = read('direct', 0);
near(direct.listCorrection, 0, 1e-9, 'heel correction upright, volume-correction tank');
near(direct.volumeObserved, 374.3814, 0.0005, 'volume upright, volume-correction tank');

/* Sounding-correction tanks: the correction lands on the ullage, and the
   capacity table is read there. Every figure below is a hand calculation. */
near(read('correction', 0).volumeObserved, 374.3814, 0.0005, 'volume upright');
const heeled = read('correction', 1);
...
/* Half a degree now runs 0 -> 1 deg, not -1 -> +1 deg. */
near(read('correction', 0.5).listCorrection, -27.6318, 0.0005, 'heel correction at half a degree');
