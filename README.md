...
Electron loads the unified gateway locally on sticky loopback ports so license and Voyage IndexedDB survive relaunch; portable mode stores all databases beside the EXE under `ChEngAIO-data/` (server JSON + Electron profile / IndexedDB) so a USB stick is fully standalone.
